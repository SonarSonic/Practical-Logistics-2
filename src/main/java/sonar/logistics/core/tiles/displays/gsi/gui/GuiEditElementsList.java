...
public class GuiEditElementsList extends GuiAbstractEditGSI implements IGridGui {
	...
	public void renderGridElement(int gridID, Object obj, int x, int y, int slot) {
		if (obj instanceof IDisplayElement) {
			...
			if (selected.contains(element)) {
				...
				drawTransparentRect(0, 0, defgridElementWidth - 2, defgridElementHeight, PL2Colours.getDefaultSelection().getRGB());
			}
			...
			GlStateManager.disableLighting();
			drawTransparentRect(0, 0, defgridElementWidth - 2, defgridElementHeight, PL2Colours.category.getRGB());
	...
	public void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) {
		...
		GlStateManager.disableLighting();
		grids.forEach((grid, scroll) -> {
			drawTransparentRect(guiLeft + grid.xPos - 1, guiTop + grid.yPos - 1, guiLeft + grid.xPos + (grid.eWidth * grid.gWidth) - 1, guiTop + grid.yPos + (grid.eHeight * grid.gHeight) - 1, PL2Colours.grey_base.getRGB());
			drawTransparentRect(guiLeft + grid.xPos, guiTop + grid.yPos, guiLeft + grid.xPos + (grid.eWidth * grid.gWidth) - 2, guiTop + grid.yPos + (grid.eHeight * grid.gHeight) - 2, PL2Colours.blue_overlay.getRGB());
		});
	}
