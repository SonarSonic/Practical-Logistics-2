...
public class DataEmitterPart extends WirelessPart implements IDataEmitter, IFlexibleGui, IByteBufTile {
	...
	@Override
	public MonitoredList<IMonitorInfo> getUpdatedList(InfoUUID uuid, Map<NodeConnection, MonitoredList<IMonitorInfo>> channels, List<NodeConnection> usedChannels) {
		MonitoredList updateList = MonitoredList.newMonitoredList(getNetworkID());
		for (Entry<NodeConnection, MonitoredList<IMonitorInfo>> entry : channels.entrySet()) {
