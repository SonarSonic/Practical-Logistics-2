...
public class WIPListNetworkChannels<M extends IMonitorInfo, H extends INetworkListHandler<M>> extends DefaultNetworkChannels<H> implements INetworkListChannels<H> {
	...
	private List<INetworkListChannels<H>> subChannels; // connections which take connections from this one
	private List<INetworkListChannels<H>> masterChannels; // connections which give connections to this one
	...
	public WIPListNetworkChannels(H handler, ILogisticsNetwork handling) {
		super(handler, handling, CacheHandler.READER);
	}
	...
	public void checkAndLinkChannels() {
		handling.getListenerList().getListeners(ILogisticsNetwork.WATCHING_NETWORK).forEach(handling -> {
			INetworkChannels channels = handling.getNetworkChannels(handler);
			if (channels != null && channels.getNetwork().isValid()) {
				((INetworkListChannels<H>) channels).addMasterChannels(this);
			}
		});
		handling.getListenerList().getListeners(ILogisticsNetwork.CONNECTED_NETWORK).forEach(handling -> {
			INetworkChannels channels = handling.getNetworkChannels(handler);
			if (channels != null && channels.getNetwork().isValid()) {
	...
	protected void updateTicks() {
		...
		if (hasListeners) {
			this.currentList = handler.getChannelsList(handling, readers);
	...
	public void createChannelLists() {
		checkAndLinkChannels();
		localInfo = handler.getAllChannels(new HashMap<>(), handling);
		updateTicks();
	}
	...
	private void updateReaders() {
		...
		while (readerIterator.hasNext() && used != readersPerTick) {
			...
			if (reader.getListenerList().hasListeners()) {
				handler.updateAndSendList(handling, reader, fullInfo, true);
			}
			used++;
		}
	}
	...
	private void updateAllReaders(boolean send) {
		readers.forEach(reader -> handler.updateAndSendList(handling, reader, fullInfo, send));
	}
	...
	public void sendLocalRapidUpdate(IListReader<M> reader, EntityPlayer viewer) {
		...
		if (listener != null) {
			updateAllChannels(true);
			Pair<InfoUUID, MonitoredList<M>> list = handler.updateAndSendList(handling, reader, fullInfo, false);
