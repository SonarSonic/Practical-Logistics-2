...
public class GuiLogistics extends GuiSonarTile {
	...
	public void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) {
		...
		drawTransparentRect(this.guiLeft, this.guiTop, this.guiLeft + this.xSize, this.guiTop + this.ySize, PL2Colours.layers[1].getRGB());
		drawTransparentRect(this.guiLeft + 1, this.guiTop + 1, this.guiLeft + this.xSize - 1, this.guiTop + this.ySize - 1, PL2Colours.layers[2].getRGB());
	...
	public void renderScroller(SonarScroller scroller) {
		if (scroller != null && scroller.renderScroller) {
			...
			if (scroller.orientation.isVertical()) {
				...
				drawTransparentRect(scroller.left, scroller.top, scroller.left + 8, scroller.top + scroller.length - 2, PL2Colours.layers[1].getRGB());
				drawTransparentRect(scroller.left, scrollYPos, scroller.left + 8, scrollYPos + 15, PL2Colours.layers[2].getRGB());
				drawTransparentRect(scroller.left, scrollYPos, scroller.left + 8, scrollYPos + 15, PL2Colours.layers[2].getRGB());
				drawTransparentRect(scroller.left, scrollYPos, scroller.left + 8, scrollYPos + 15, PL2Colours.layers[2].getRGB());
			} else {
				int scrollXPos = scroller.left + (int) ((float) (scroller.width - 15) * scroller.getCurrentScroll());
				drawTransparentRect(scroller.left, scroller.top, scroller.left + scroller.width, scroller.top + scroller.length - 2, PL2Colours.layers[1].getRGB());

				drawTransparentRect(scrollXPos, scroller.top, scrollXPos + 15, scroller.top + scroller.length - 2, PL2Colours.layers[2].getRGB());
				drawTransparentRect(scrollXPos, scroller.top, scrollXPos + 15, scroller.top + scroller.length - 2, PL2Colours.layers[2].getRGB());
				drawTransparentRect(scrollXPos, scroller.top, scrollXPos + 15, scroller.top + scroller.length - 2, PL2Colours.layers[2].getRGB());
			}
		}
