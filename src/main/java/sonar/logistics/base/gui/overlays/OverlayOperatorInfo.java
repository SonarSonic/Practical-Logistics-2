...
public class OverlayOperatorInfo {
	...
	public static void tick(DrawBlockHighlightEvent evt) {
		...
		if (provider != null) {
			...
			GuiSonar.drawTransparentRect(-maxWidth / 2, -maxHeight / 2, maxWidth / 2, maxHeight / 2, PL2Colours.layers[1].getRGB());
			GuiSonar.drawTransparentRect(-maxWidth / 2 + 1, -maxHeight / 2 + 1, maxWidth / 2 - 1, maxHeight / 2 - 1, PL2Colours.layers[2].getRGB());
			GuiSonar.drawTransparentRect(-maxWidth / 2 + 1, -maxHeight / 2 + 1, maxWidth / 2 - 1, maxHeight / 2 - 1, PL2Colours.layers[2].getRGB());
			GuiSonar.drawTransparentRect(-maxWidth / 2 + 1, -maxHeight / 2 + 1, maxWidth / 2 - 1, maxHeight / 2 - 1, PL2Colours.layers[2].getRGB());
