...
public class GuiFilterList extends GuiSelectionList {
	...
	public void drawGuiContainerForegroundLayer(int x, int y) {
		...
		case ITEM_FILTER:
			...
			break;
		case LIST:
			// FontHelper.textCentre(FontHelper.translate("Filters"), xSize, 6, PL2Colours.white_text);
			// FontHelper.textCentre(String.format("Select the filter you wish to edit"), xSize, 18, PL2Colours.grey_text);
			break;
		case ORE_FILTER:
	...
	public void drawInfo(int pos, DisplayInfo info) {
		...
		if (monitorInfo instanceof INameableInfo) {
			...
			FontHelper.text(directInfo.getClientIdentifier(), 11, top + 6, PL2Colours.white_text.getRGB());
		} else {
			FontHelper.text("NO DATA", 11, top + 6, PL2Colours.white_text.getRGB());
		}
	}
	*/
	@Override
	...
	public void drawGuiContainerBackgroundLayer(float var1, int var2, int var3) {
		...
		if (state == GuiState.FLUID_FILTER || state == GuiState.ITEM_FILTER) {
			drawTransparentRect(guiLeft + 12, guiTop + 31, guiLeft + 228, guiTop + 157, PL2Colours.grey_base.getRGB());
			drawTransparentRect(guiLeft + 13, guiTop + 32, guiLeft + 227, guiTop + 156, PL2Colours.blue_overlay.getRGB());
		}
		drawTransparentRect(guiLeft + 12, guiTop + 170, guiLeft + xSize - 12, guiTop + 252, PL2Colours.grey_base.getRGB());
		drawTransparentRect(guiLeft + 13, guiTop + 171, guiLeft + xSize - 13, guiTop + 251, PL2Colours.blue_overlay.getRGB());
	}
