package sonar.logistics.integration.extremereactors;
/* FIXME
import java.util.List;

import erogenousbeef.bigreactors.base.multiblock.MultiblockReactor;
import erogenousbeef.bigreactors.base.multiblock.tileentity.TileEntityReactorPart;
...
import sonar.logistics.connections.monitoring.InfoNetworkFluid;
...
public class BigReactorsCoolantHandler implements ICustomTileHandler {
	...
	public void addInfo(ILogicInfoRegistry registry, List<IProvidableInfo> infoList, LogicPath currentPath, Integer methodCode, World world, IBlockState state, BlockPos pos, EnumFacing dir, Block block, TileEntity tile) {
		...
		if (reactor != null && !reactor.isPassivelyCooled()) {
			...
			if (vapor != null) {
				infoList.add(new InfoNetworkFluid(new StoredFluidStack(vapor, reactor.getCoolantContainer().getVaporAmount())).setPath(currentPath.dupe()));
			}
			Fluid coolant = reactor.getCoolantContainer().getCoolantType();
			if (fluid != null) {
				infoList.add(new InfoNetworkFluid(new StoredFluidStack(fluid, reactor.getCoolantContainer().getCoolantAmount())).setPath(currentPath.dupe()));
			}
		}
